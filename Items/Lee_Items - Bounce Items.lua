...
local index = index + 1 -- increment
	...
	-- First loop: Set all items offline (prevents file locking issues when renaming multiple files)
	...
	-- Second loop: Rename files and reassign sources
	for _, v in ipairs(rend_trks_t) do
		...
		if take then
			...
			if old_fn and old_fn ~= '' then
				...
				if f_path and f_name then
					...
					-- Rename file with error checking
					local ok, message = os.rename(old_fn, new_fn)
					if ok then
						-- Create new source and assign it
						...
						if new_src then
							...
							-- If source creation failed, try to restore original
							r.PCM_Source_CreateFromFile(old_fn)
							os.rename(new_fn, old_fn) -- restore original filename
						end
					else
						-- If rename failed, keep original file
						-- Source should still be valid
					end
					
					-- Remove peak files
					local peak_fn = r.GetPeakFileName(f_path..f_name, '')
					if peak_fn and peak_fn ~= '' then
						os.remove(peak_fn)
					end
	...
	-- Third loop: Set all items back online
	for _, v in ipairs(rend_trks_t) do
		for i = 0, r.CountTrackMediaItems(v)-1 do
		local item = r.GetTrackMediaItem(v, i)
		ACT(40289) -- Item: Unselect all items
