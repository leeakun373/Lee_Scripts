--[[
  Theme System for Marker Workstation
...
function Themes.applyTheme(ctx, theme)
    ...
    -- Tab 选项卡颜色 (使用 REAPER ImGui 实际支持的常量名称)
    if theme.TAB then
    ...
    -- 注意：REAPER ImGui 可能使用 TabSelected 而不是 TabActive
    if theme.TAB_ACTIVE then
        -- 尝试使用 TabSelected（如果存在）
    ...
    -- TabUnfocused 和 TabUnfocusedActive 可能不存在，先注释掉
    -- 如果需要，可以使用 TabDimmed 和 TabDimmedSelected（如果存在）
...
-- Modern Studio Theme (Muted - 低饱和度，Marker/Region 颜色区分)
Themes.modern = {
    name = "Modern",
    -- Button colors (custom - Marker 琥珀色，Region 青色，区分明确)
    BTN_MARKER_ON  = 0xB45309FF,  -- Marker 激活时使用哑光琥珀色 (Amber-700)
    BTN_MARKER_OFF = 0x27272AFF,  -- 非激活使用深灰
    BTN_REGION_ON  = 0x0F766EFF,  -- Region 激活时使用深青色 (Teal-700)
    BTN_REGION_OFF = 0x27272AFF,  -- 非激活使用深灰
    BTN_RELOAD     = 0x27272AFF,  -- 普通按钮，使用深灰
    BTN_MANAGER    = 0x27272AFF,  -- 普通按钮，使用深灰
...
Themes.studio = {
    name = "Studio",
    -- Button colors (custom - NVK 风格)
    BTN_MARKER_ON  = 0x1565C0FF,  -- Marker 激活时使用 NVK 蓝色
    BTN_MARKER_OFF = 0x252525FF,  -- 非激活使用深灰
    BTN_REGION_ON  = 0x1565C0FF,  -- Region 激活时使用 NVK 蓝色
    BTN_REGION_OFF = 0x252525FF,  -- 非激活使用深灰
    BTN_RELOAD     = 0x252525FF,  -- 普通按钮，使用深灰
    BTN_MANAGER    = 0x252525FF,  -- 普通按钮，使用深灰
