--[[
  Marker Function: Move Nearest Marker to Cursor
...
local function execute()
    ...
    reaper.DeleteProjectMarker(proj, nearestMarker.index, false)
    reaper.AddProjectMarker2(proj, false, cursorPos, 0, nearestMarker.name, -1, markerColor)
    
    reaper.UpdateArrange()
    reaper.PreventUIRefresh(-1)
    reaper.Undo_EndBlock("Move nearest marker to cursor", -1)
    
    return true, string.format("Success: Moved marker '%s' to cursor (distance: %.2fs)", nearestMarker.name, distance)
end

-- Return module
return {
    name = "Move to Cursor",
    description = "Move nearest marker to cursor position",
    execute = execute,
    buttonColor = {0xFF8800FF, 0xFF9933FF, 0xFF6600FF}  -- Orange
}
