...
function LayoutManager.initializeLayout(layout, marker_functions, custom_actions)
    local was_empty = (#layout.active == 0)
    local added_new = false
    
    -- Add all functions to active (if first time, or if new functions exist)
    for _, func in ipairs(marker_functions) do
        if func and func.name then
            local already_added = false
            -- Check if in active
            for _, active_name in ipairs(layout.active) do
            ...
            if not already_added then
                for _, stash_name in ipairs(layout.stash) do
                    if stash_name == func.name then
                        already_added = true
                        break
                    end
                end
            ...
            if not already_added then
                table.insert(layout.active, func.name)
                added_new = true
            end
        end
    end
    ...
    for _, action in ipairs(custom_actions) do
        if action and action.name and action.action_id then
            ...
            if action_id then
                ...
                -- Add to active if not found anywhere
                if not already_added then
                    table.insert(layout.active, action.name)
                    added_new = true
                end
            end
        end
...
function LayoutManager.applyLayoutPreset(layout_presets, preset_name)
    if layout_presets[preset_name] then
        ...
        for _, func_name in ipairs(preset.active) do
            table.insert(new_layout.active, func_name)
        end
        for _, func_name in ipairs(preset.stash) do
            table.insert(new_layout.stash, func_name)
        end
        return new_layout
    end
    return nil
...
function LayoutManager.saveCurrentLayoutAsPreset(layout, layout_presets, preset_name)
    if preset_name and preset_name ~= "" then
        local active_copy = {}
        local stash_copy = {}
        for _, func_name in ipairs(layout.active) do
            table.insert(active_copy, func_name)
        end
        for _, func_name in ipairs(layout.stash) do
...
function LayoutManager.moveToStash(layout, func_name)
...
function LayoutManager.moveToActive(layout, func_name, position)
    -- Remove from stash
    ...
    if not already_added then
        if position and position > 0 and position <= #layout.active then
            table.insert(layout.active, position, func_name)
        else
...
function LayoutManager.reorderFunctions(layout, src_name, dst_name)
    ...
    if src_idx and dst_idx and src_idx ~= dst_idx then
        ...
        table.insert(layout.active, adjusted_dst, src_name)
        return true
    end
    return false
end

-- Clean up invalid function names
function LayoutManager.cleanupLayout(layout, getFunctionByName)
    local valid_active = {}
    for _, func_name in ipairs(layout.active) do
    ...
    for _, func_name in ipairs(layout.stash) do
        if func_name and func_name ~= "" then
            local func = getFunctionByName(func_name)
            if func and func.name then
                table.insert(valid_stash, func_name)
            end
        end
    end
