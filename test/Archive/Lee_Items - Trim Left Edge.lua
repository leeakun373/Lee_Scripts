...
-- 辅助函数：查找轨道上cursor位置后的第一个item（在arrange view范围内）
local function next_track_item_in_arrangeview(track, pos)
    ...
    for i = 0, item_count - 1 do
        ...
        if item then
            ...
            if item_start >= pos and item_start < arrange_end then
                return item
            end
        end
    end
...
-- 辅助函数：处理音量自动化（Trim Left）
local function trim_volume_automation_from_left(item, cursor_pos, init_item_pos)
    ...
    local env = reaper.GetTakeEnvelopeByName(reaper.GetActiveTake(item), "Volume")
    if not env then return end
    
    local diff = cursor_pos - init_item_pos
    ...
    for ai = 0, ai_count - 1 do
        ...
        -- 如果automation item在trim范围内
        if ai_pos < cursor_pos and ai_end > init_item_pos then
            if ai_pos < init_item_pos then
                -- automation item开始于item之前，需要调整startoffset
                local startoffs = reaper.GetSetAutomationItemInfo(env, ai, "D_STARTOFFS", 0, false)
                local playrate = reaper.GetSetAutomationItemInfo(env, ai, "D_PLAYRATE", 0, false)
                local trim_amount = math.min(cursor_pos - init_item_pos, ai_end - init_item_pos)
                reaper.GetSetAutomationItemInfo(env, ai, "D_STARTOFFS", startoffs + trim_amount * playrate, true)
                reaper.GetSetAutomationItemInfo(env, ai, "D_LENGTH", ai_len - trim_amount, true)
            else
                -- automation item开始于item之后，只需要移动位置
                reaper.GetSetAutomationItemInfo(env, ai, "D_POSITION", cursor_pos, true)
            end
        end
    end
end
...
if not init_item then
    ...
    if track and track_context == 2 then
        init_item = next_track_item_in_arrangeview(track, cursor_pos)
    end
end
if not init_item then
...
-- 计算init_pos（参考nvk：排除第一个item，只计算非mute items中最左侧的位置）
...
if init_pos == math.huge then
    init_pos = reaper.GetMediaItemInfo_Value(items[1], "D_POSITION")
end
...
for i, item in ipairs(items) do
    ...
    if diff <= init_diff + 0.0001 or diff < 0 or (#items > 1 and i == 1) then
        ...
        elseif item_end > cursor_pos then
            -- 手动处理不可见轨道
        ...
        -- 处理音量自动化
        trim_volume_automation_from_left(item, cursor_pos, init_item_pos)
        
        -- 保持淡入长度不变（用户要求）
        if fadein_len > 0.0001 then
            local restored_fadein_len = math.min(fadein_len, item_length)
