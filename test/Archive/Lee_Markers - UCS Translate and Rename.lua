...
function Loop()
    ...
    if visible then
        ...
        -- 语言切换按钮
        local lang_btn_col = (app_state.display_language == "en") and COLORS.BTN_MODE_ON or COLORS.BTN_MODE_OFF
        local lang_btn_txt = (app_state.display_language == "en") and "Language: EN" or "Language: 中文"
        PushBtnStyle(lang_btn_col)
        if r.ImGui_Button(ctx, lang_btn_txt) then
        ...
        if r.ImGui_BeginChild(ctx, "table_area", 0, -footer_h, c_flags) then
            ...
            if r.ImGui_BeginTable(ctx, table_id, num_cols, t_flags) then
                ...
                r.ImGui_TableSetupColumn(ctx, orig_header, r.ImGui_TableColumnFlags_WidthStretch(), 1.2)
                
                -- FXName列
                r.ImGui_TableSetupColumn(ctx, 'FXName', r.ImGui_TableColumnFlags_WidthStretch(), 1.0)
                
                -- 可选字段列（动态添加，仅在UCS模式下且字段可见时）
                if app_state.use_ucs_mode then
                    if app_state.field_visibility.vendor_category then
                        r.ImGui_TableSetupColumn(ctx, 'Vendor Category', r.ImGui_TableColumnFlags_WidthFixed() | r.ImGui_TableColumnFlags_NoResize(), 100)
                    end
                    if app_state.field_visibility.creator_id then
                        r.ImGui_TableSetupColumn(ctx, 'CreatorID', r.ImGui_TableColumnFlags_WidthFixed() | r.ImGui_TableColumnFlags_NoResize(), 100)
                    end
                    if app_state.field_visibility.source_id then
                        r.ImGui_TableSetupColumn(ctx, 'SourceID', r.ImGui_TableColumnFlags_WidthFixed() | r.ImGui_TableColumnFlags_NoResize(), 100)
                    end
                    if app_state.field_visibility.user_data then
                        r.ImGui_TableSetupColumn(ctx, 'User Data', r.ImGui_TableColumnFlags_WidthFixed() | r.ImGui_TableColumnFlags_NoResize(), 100)
                    end
                end
                
                -- Preview列
                if app_state.use_ucs_mode then
                    r.ImGui_TableSetupColumn(ctx, 'Preview', r.ImGui_TableColumnFlags_WidthStretch(), 2.0)
                else
                ...
                if app_state.use_ucs_mode then
                    ...
                    if app_state.field_visibility.vendor_category then
                        ...
                        -- 手动绘制表头文本和按钮，避免重叠
                        r.ImGui_Text(ctx, 'Vendor Category')
                        r.ImGui_SameLine(ctx, 0.0, 4.0)
                        PushBtnStyle(COLORS.BTN_COPY)
                        if r.ImGui_SmallButton(ctx, "Fill") then
                    ...
                    if show then
                        ...
                        if app_state.use_ucs_mode then
                            ...
                            if item.ucs_cat_id and item.ucs_cat_id ~= "" then
                                r.ImGui_TextColored(ctx, COLORS.TEXT_NORMAL, item.ucs_cat_id)
                            else
                                r.ImGui_TextDisabled(ctx, "-")
                            end
                            -- 使用固定位置对齐按钮（按钮始终在同一位置）
                            r.ImGui_SameLine(ctx, 60.0, 4.0)  -- 固定位置60像素
                            PushBtnStyle(COLORS.BTN_AUTO)
                            if r.ImGui_SmallButton(ctx, "Auto") then AutoMatchItem(item) end
                            r.ImGui_PopStyleColor(ctx, 3)
                        end
