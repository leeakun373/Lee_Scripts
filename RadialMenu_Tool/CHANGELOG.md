...
### 修复
- 🐛 **窗口位置漂移问题**：修复右键切换管理模式时窗口位置发生变化的问题
  - 窗口位置现在保持在最初唤出时的鼠标位置，不再因模式切换而重新定位
  - 移除模式切换时的坐标捕获逻辑，使用固定的 target_gui_pos
- 🐛 **子菜单瞬间闪现**：添加重定位冻结保护机制（0.1秒）
  - 在模式切换后的0.1秒内禁用扇区判定，防止坐标剧烈变化导致的误触发
  - 确保切换模式时不会因为窗口重定位而触发子菜单展开

### 改进
- 🎯 **右键响应范围扩大**：内圆全域响应右键点击，不再局限于 InvisibleButton 区域
  - 通过数学距离计算判断鼠标是否在内圆范围内
  - 提升用户体验，更容易触发管理模式切换
- 🎨 **预设创建优化**：空白预设现在默认创建2个扇区（最小扇区数改为2）
  - 扇区数量滑轨最小值从1改为2，避免单扇区配置
  - 统一配置结构，提升一致性

### 技术改进
- 🔧 **窗口定位系统重构**：实现方案A+B结合的智能定位策略
  - 方案A：未烘焙时首帧离屏透明烘焙
  - 方案B：已烘焙时使用缓存位置数据直接定位
  - 确保窗口在第一帧就在正确位置，无闪现无跳动
- 🔧 **坐标锁定机制**：在首次唤出时捕获鼠标位置并锁定
  - 后续所有模式切换都基于这个固定坐标，不会跟随鼠标移动
