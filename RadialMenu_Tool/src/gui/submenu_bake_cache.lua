...
function M.bake_submenus(ctx, center_x, center_y, outer_radius, config)
    ...
    -- 从配置读取布局参数，如果没有则使用默认值
    local slot_w = (config and config.menu and config.menu.slot_width) or 60   -- 按钮宽度
    local slot_h = (config and config.menu and config.menu.slot_height) or 25   -- 按钮高度
    local gap = (config and config.menu and config.menu.submenu_gap) or 3       -- 按钮间距
    local padding = (config and config.menu and config.menu.submenu_padding) or 4   -- 窗口内边距
    local cols = 4      -- 网格列数（与 list_view.lua 中的 GRID_COLS 常量一致）
    ...
    local edge_padding = 20  -- 边缘留白
    local min_x = -outer_radius - edge_padding
    local min_y = -outer_radius - edge_padding
    local max_x = outer_radius + edge_padding
    local max_y = outer_radius + edge_padding
    ...
    min_x = min_x - edge_padding
    min_y = min_y - edge_padding
    max_x = max_x + edge_padding
    max_y = max_y + edge_padding
    ...
    for _, sector in ipairs(config.sectors) do
        if sector.slots and #sector.slots > 0 then
            ...
            -- 【核心】提前算好所有东西！（使用相对坐标）
            -- 【修复】背景框覆盖整个窗口（包含 padding），这样背景框能正常显示
            -- padding 的效果通过按钮位置体现（按钮距离窗口边缘有 padding 的距离）
            local cached_sector = {
                -- 背景框坐标（相对于圆心）- 覆盖整个窗口，包含 padding
